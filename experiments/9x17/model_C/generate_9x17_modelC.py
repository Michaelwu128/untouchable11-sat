...
num_cell_vars = BOARD_H * BOARD_W * NUM_PIECES # 9 * 17 * 11
...
def get_cell_var(r, c, p):
    ...
    cell_index = (r * BOARD_W) + c # BOARD_W = 17
...
for p_idx in range(NUM_PIECES):
    ...
    if not plcs:
        raise SystemExit(f"錯誤：拼圖 {p_idx} 在 9x17 棋盤上沒有任何合法放置位置！")
...
print(f"接下來可以用 cadical {cnf_name} solution_9x17_amo.txt 求解")