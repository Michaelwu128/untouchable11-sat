#!/usr/bin/env python3
# generate_12x12_final.py
#
# 12x12 混合式 + AMO (循序計數器)
# *** 最終優化 ***
# 加入「破除對稱性」(Symmetry Breaking) 限制
# 來加速 (很可能的) UNSAT 證明。
    ...
    for f_idx, t in enumerate(transforms):
        ...
        for x in range(BOARD_H - th + 1):
            for y in range(BOARD_W - tw + 1):
                ...
                # vvvvvvvvvvvvvvvv [ 新增的加速程式碼 ] vvvvvvvvvvvvvvvv
                #
                # *** 破除對稱性 (Symmetry Breaking) ***
                # 我們強迫「拼圖 0」(隨便選一塊) 
                # 只能被放置在棋盤的上半部 (x <= 6)。
                # 這會將搜索空間砍掉一半，大幅加速 UNSAT 證明。
                #
                if p_idx == 0:
                    if x > (BOARD_H // 2):
                        continue # 如果 p0 的 x 座標不在上半部，就跳過 (不產生這個變數)
                #
                # ^^^^^^^^^^^^^^^^ [ 新增的加速程式碼 ] ^^^^^^^^^^^^^^^^
...
print(f"  > 找到 {len(placements)} 個合法放置 (已破除對稱性), 分配變數 ID 1..{num_placement_vars}")
...
for p_idx in range(NUM_PIECES):
    ...
    if not plcs:
        # 如果因為破除對稱性導致 p0 沒地方放 (極不可能)，這裡會報錯
        raise SystemExit(f"錯誤：拼圖 {p_idx} 在 (限制後的) 12x12 棋盤上沒有任何合法放置位置！")
...
# --- 階段 4: 寫入檔案
# ----------
cnf_name = f"untouchable_{BOARD_H}x{BOARD_W}_final.cnf"
placements_name = f"placements_{BOARD_H}x{BOARD_W}_final.txt"
...
print(f"接下來可以用 cadical {cnf_name} solution_12x12_final.txt 求解")
print("這個版本移除了對稱性，證明 UNSAT 的速度會*顯著*加快。")